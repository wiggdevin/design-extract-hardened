...
The owner comes from the element's ARIA role, tag, class names, and box, in that order of trust. Class names are split into tokens on punctuation, underscores, and case changes, so `Card_bgWhite`, `GkoSzG_panel`, and `Section__cta` expose their `card`, `panel`, and `cta` tokens (CSS modules and camelCase hid them from a word-boundary match). A form field (`input`, `textarea`, `select`, or `role=textbox`) is an input whatever its class says; a toggle (`role=checkbox|radio|switch`) is a badge, because a 20px box says nothing about the field system. Prose and headings are text whatever their class says. A `<button>` is a button even when its class names the menu or section around it. A link that paints its own box, has at least 8px of horizontal padding, and sits in a control's size range (24 to 96px tall, at most 640px wide) is a button with no class token at all, which is how Framer and Tailwind sites mark their buttons; a link with a Button component's class but no fill and no padding is a text link. Inside a role, each element votes with its side length (the square root of its area, clamped between 16 and 320), so fifteen 330×40 CTAs outweigh twelve 190×30 variant toggles and a full-screen overlay cannot swamp a role. A field that leaves painting to a wrapper (a transparent `<input>` inside a rounded box) still casts no vote; that is the known gap.
