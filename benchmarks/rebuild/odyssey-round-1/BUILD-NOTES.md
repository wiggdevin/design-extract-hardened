...
- **Bands 5, 6, 8, 9 media (`media.src`) are transparent placeholder SVGs**, not real captured photography — e.g. band 5's `data:image/svg+xml,...` decodes to a `724x668` rect with `fill-opacity="0"` (fully transparent), same pattern for bands 6, 8, 9. The extraction did not capture real imagery for these bands (likely lazy-loaded/background-image assets the crawler didn't resolve). I did not literally embed the transparent data URIs (an invisible image would misrepresent "media kind: photo" to a reviewer); instead each of those slots renders as a dashed/gradient placeholder block sized to the recorded aspect ratio, and this is called out inline via the `.media-placeholder` class. This is a direct instance of "the extraction did not tell me what I needed" — there is no real photo to reference or reason about (composition, subject, crop).
